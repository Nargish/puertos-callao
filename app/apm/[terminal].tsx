...
// hooks de datos (ya actualizados en src/features/lanes/hooks.ts)
import {
  useIsAdmin,
  useTerminal,
  useLanes,
...
const LANE_AREA_H = Math.round(SCREEN_H * 0.33); // altura de carriles (chatito)

export default function TerminalScreen() {
  const { terminal } = useLocalSearchParams<{ terminal?: string }>();
  const insets = useSafeAreaInsets();

  // 1) Datos del terminal por nombre mostrado (e.g. "Manco Cápac")
  const { data: term, isLoading: loadingTerm, error: errTerm } = useTerminal(
    terminal || ""
  );

  // 2) Lanes del terminal + realtime
  const {
    data: lanesRaw,
    refetch,
    isFetching,
  ...
  const GAP = 4;
  const HEADER_H = 18; // fila C1..C11

  // Orden estable de carriles:
  const lanes = useMemo(() => {
    const arr = lanesRaw ?? [];
    // si tu tabla tiene idx, ideal: ordenar por idx en el server; si no, ordena por nombre "CARRIL 01.."
  ...
  // Métrica para que los bloques (camiones) llenen de arriba hacia abajo
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }} edges={["top"]}>
      ...
      >
        ...
        {/* Fila de acciones: Actualizar + (badge admin) */}
        <View
          style={{
            flexDirection: "row",
      ...
      <View onLayout={onLayout} style={{ height: LANE_AREA_H, paddingHorizontal: 8 }}>
        ...
        <View style={{ flex: 1, flexDirection: "row", gap: 6 }}>
          {lanes.map((lane) => {
            ...
            // blockH por carril, calculado para que 'cap' bloques llenen el alto útil
            const blockH =
              cap > 0
            ...
            return (
              ...
              >
                {/* Bloques desde ARRIBA hacia abajo */}
                <View
                  style={{
                    flex: 1,
      ...
      {/* Panel de controles SOLO para admin */}
      {isAdmin && (
        <ScrollView
          horizontal
